---
title: "MongoDB Data Models: Users and Posts"
...
On the **first invocation** of `dbConnect()`, `cached.conn` is `null` so Mongoose initiates a new connection. The pending `Promise` is stored in `cached.promise` and the resolved `Connection` in `cached.conn`. On every **subsequent invocation** within the same Node.js process (i.e., warm serverless function), `cached.conn` is already populated and the function returns immediately — no new connection is opened.
